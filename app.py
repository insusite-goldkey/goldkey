# ==========================================================================
# 👑 [골드키지사 AI 마스터 - 최신 SDK 최적화 및 무손실 완결본]
# 절대규칙: 효율을 위해 섹션을 통합하지 말 것. 각 섹션은 독립된 효능을 가진다.
# 1. SDK 동기화: AI Studio 권고에 따라 최신형 라이브러리 호출 규격 적용.
# 2. 404 원천 봉쇄: 모델 객체 생성 시 경로 자동 매칭 로직 탑재.
# 3. 사이드바 완벽 복구: 8년 혜택, 관리자 프로필, API 튜토리얼 무손실 유지.
# 4. 섹션 독립: 1~15번 섹션을 물리적(st.divider)으로 철저히 분리.
# ==========================================================================
...
# --------------------------------------------------------------------------
# [SECTION 2] 최신 마스터 엔진 (SDK 구형 경고 해결판)
# --------------------------------------------------------------------------
SYSTEM_PROMPT = """당신은 이세윤 관리자가 설계한 '골드키지사 AI 마스터'입니다.
1단계: 판례, 금감원 사례, CFP 기준 답변. 2단계: 구글 실시간 검색 검증.
로직: 건보료 기반 소득 역산(0.0709) 및 적정 보험료 15% 가이드를 준수하라."""

def get_master_model(api_key):
    """최신 SDK 규격에 맞춰 모델을 로드하는 고효능 함수"""
    try:
        genai.configure(api_key=api_key)
        # 최신 SDK에서는 'gemini-1.5-flash'만 입력하면 내부에서 주소를 자동 완성합니다.
...
# --------------------------------------------------------------------------
# [SECTION 3] 왼쪽 사이드바: 마스터 회원 센터 (무손실 복구)
# --------------------------------------------------------------------------
with st.sidebar:
    st.header("🔑 마스터 회원 센터")
    ...
    st.markdown("### 🌐 API 관리 센터")
    st.markdown('<a href="https://aistudio.google.com/app/apikey" target="_blank"><button style="width:100%; cursor:pointer; background:#1E88E5; color:white; border:none; padding:10px; border-radius:5px; font-weight:bold;">🌐 API 키 발급/확인</button></a>', unsafe_allow_html=True)
...
if not st.session_state.login_status:
    st.warning("🔒 로그인이 필요합니다. (admin / gold1234)"); st.stop()
...
with st.expander("💡 실전 보상 & 민원 대응 (판례 2001다1480)"):
    st.write("보험사의 설명의무 위반 시 대응 법리입니다.")
...
ess_reply = st.text_input("일상배책, 화재, 자동차, 운전자, 통합보험 상태", key="ess_q")

# --------------------------------------------------------------------------
# [SECTION 7] 2단계: 전문 증권 분석 자료 요청 (獨立)
# --------------------------------------------------------------------------
st.divider(); st.write("### 📸 2단계: 전문 증권 분석 자료 요청")
uploaded_files = st.file_uploader("증권 파일 업로드", accept_multiple_files=True, key="up_main")
...
if nhi_premium > 0:
    ...
    st.success(f"📊 역산 월 소득: **{calc_income:,.0f}원** / 적정 보험료: **{calc_income*0.15:,.0f}원**")

# --------------------------------------------------------------------------
# [SECTION 9] 4단계: 질병 보상 정밀 분석 및 가족력 (獨立)
# --------------------------------------------------------------------------
st.divider(); st.write("### 🏥 4단계: 질병 보상 정밀 분석 및 가족력")
dis_text = st.text_area("가족력 및 염려 질환", key="dis_q")
dis_files = st.file_uploader("질병 증권 업로드", accept_multiple_files=True, key="up_dis")

# --------------------------------------------------------------------------
# [SECTION 10] 5단계: 대형 생보사 헬스케어 컨설팅 (獨立)
# --------------------------------------------------------------------------
st.divider(); st.write("### 💎 5단계: 대형 생보사 헬스케어 컨설팅")
hc_ans = st.radio("상급병원 진찰 예약 필요성", ["예", "아니오", "미정"], key="hc_q")

# --------------------------------------------------------------------------
# [SECTION 11] 6대 법령 및 보상 지식 DB (獨立)
# --------------------------------------------------------------------------
st.divider(); st.write("### 🏛️ 6대 법령 및 보상 지식 DB")
st.info("민법, 상법, 보험업법 판례 엔진 가동")
...
# --------------------------------------------------------------------------
# [SECTION 15] 전문가 통합 분석 실행 (獨立)
# --------------------------------------------------------------------------
st.divider()
if q_analyze:
    if not st.session_state.user_api_key: st.error("API Key 미연결")
    else:
        with st.spinner("🔍 최신 유료 엔진 정밀 분석 중..."):
