...
# 1. 즉시 분석 버튼: 상담창(Section 4.5) 바로 아래 소형 [🚀 즉시 분석/민원 요청] 버튼 배치.
# 2. 브랜딩: "보험 가입 상담 부터 보험금 분쟁 대응까지- 보험 AI전문 마스터 통합 시스템"
# 3. 사이드바 상시 노출: 회원 혜택 및 API 가이드/튜토리얼은 숨김 없이 상시 표현.
# 4. 섹션 통합 절대 금지: 1~15번 모든 섹션은 물리적 구분선으로 독립성을 수호한다.
# 5. 리스크 관리: 설계사 구상권 보호를 위해 보험업법 벌칙 규정은 답변에서 절대 제외한다.
# 6. 서류 로드 로직: 로드 수량 판별 상태창 및 빨간색 '전체 삭제' 버튼 상시 배치.
# 7. 명칭 고정: 2단계(국제재무설계 기준...), 3단계(국제 재무설계 기준...).
# ==========================================================================
...
[지휘 지침]:
1. 금기 사항: 보험업법상 불완전 판매 벌칙 및 설계사 구상권 관련 내용은 답변에서 절대 제외하라. (FC 보호)
2. 분쟁 대응: 판례 2001다1480 및 상법 제638조의3 근거로 법리 조문과 고객 사건을 결합하여 설명하라.
...
# ==========================================
# [SECTION 4.5] 마스터 통합 상담 센터 (즉시 분석 버튼 추가)
# ==========================================
st.divider()
...
# ==========================================
# [SECTION 14] 전문가 통합 분석 로직 (함수화하여 상하단 동시 대응)
# ==========================================
def run_master_analysis():
    st.components.v1.html(s_voice("종합 분석을 시작합니다."), height=0)
    with st.spinner("마스터의 지휘 아래 모든 섹션을 통합 분석 중입니다..."):
        try:
            ...
            [보상논리]: 판례 2001다1480 근거 지급 거절 대응 논리 생성. (※ 보험업법 벌칙 제외 필수)
...
# ==========================================
# [SECTION 15] 하단 최종 분석 버튼 및 성공 응원
# ==========================================
st.divider()
...
if st.button("🏆 관리자 이세윤 & 전문 FC 성공 응원"):
    st.balloons()
    msg = f"{user_name}님! 마스터의 지능과 국제 표준의 전문성으로 필승하십시오!"
