...
# 3. UI/UX: 섹션 5,6,7 분리 / 터치 시 Pulse 애니메이션 / API 가이드 팝업화
# 4. 음성안내: "마스터가 듣고 있어요" (음성은 전문 수애 엔진 사용)
# 5. 분석강령: 텍스트+이미지 '반드시' 동시 대조 분석 원칙 준수
# ==========================================================================
...
# ==========================================
# [SECTION 1] 기본 설정 및 페르소나 (무손실 강령 원문)
# ==========================================
...
# ==========================================
# [SECTION 3] 사이드바 (API 가이드 팝업/종료 보안)
# ==========================================
with st.sidebar:
    st.header("🔑 사용자 센터")
    ...
    with st.expander("👤 회원 등록 및 API 발급 가이드 (필독)", expanded=False):
        ...
        st.markdown("""
        1. **새 창 열기 원칙**: 반드시 본인 계정의 새 창에서 발급
        2. **지메일 보유 확인**: Google 계정 로그인 필수
        3. **결제 카드 등록 (VISA/MASTER)**: 형식적 절차이며 1일 50건 미만 상담 시 **약 8년간 무료** 사용 가능.
        """)
        st.markdown('<a href="https://aistudio.google.com/app/apikey" target="_blank" style="text-decoration: none;"><button style="width: 100%; border-radius: 5px; background-color: #1E88E5; color: white; border: none; padding: 10px; cursor: pointer; font-weight: bold;">🌐 API 키 발급 (새 창 열기)</button></a>', unsafe_allow_html=True)
...
# ==========================================
# [SECTION 6] 1단계: 고객 정보 입력 (동적 UI 분리)
# ==========================================
st.divider()
...
# ==========================================
# [SECTION 7] 2단계: 정밀 분석 자료 접수 (동적 UI 분리)
# ==========================================
st.divider()
...
with col_q:
    if st.button("🎤 마이크 켜기 (마스터 안내)"):
        load_stt_engine()
        
    user_question = st.text_area("❓ [1단계: 사유 입력] 민원, 궁금한 점을 입력하세요.", 
                                 height=150, placeholder="터치 시 파동 효과가 발생합니다.")
...
with col_img:
    st.image("https://raw.githubusercontent.com/insusite-goldkey/goldkey/main/ai_expert.png", 
             caption="마스터가 전문가 그룹과 협업할 준비를 마쳤습니다.", use_container_width=True)

# ==========================================
# [SECTION 8] 전문가 그룹 통합 분석 (텍스트+이미지 '반드시' 대조)
# ==========================================
...
# ==========================================
# [SECTION 9] 보상 실무 및 글로벌 지원 지식 DB (분리)
# ==========================================
st.divider()
st.write("### 🛡️ 보상 실무 및 글로벌 지원 지식 DB")
tab1, tab2, tab3 = st.tabs(["🏛️ 판례 및 법리", "🏢 법인/중대재해", "🌍 글로벌 보상"])
with tab1:
    st.info("🎯 **대법원 판례 2001다1480**")
    st.write("합의 당시 예상치 못한 후유증 추가 청구의 법리적 근거.")
with tab2:
    st.warning("💼 **법인 자산계상 및 배상책임**")
    st.write("해지환급금 자산계상 원칙 및 중대재해 배상 채무 상쇄 전략.")
with tab3:
    global_q = st.text_area("🌍 글로벌 보상 상황 입력 (다국어 지원)", key="global_q")
    if st.button("수예(마스터)에게 글로벌 해결 요청"):
        if global_q:
            res = genai.GenerativeModel('gemini-1.5-flash').generate_content(f"민법 제733조 근거 답변: {global_q}")
            st.success("✅ 분석 완료"); st.markdown(res.text)

# ==========================================
# [SECTION 10] 성공 응원 및 시스템 보안 (분리)
# ==========================================
st.divider()
...
with col_final1:
    if st.button("🏆 모든 FC님들의 성공을 위한 업데이트 확인", use_container_width=True):
        st.balloons()
        msg = f"{user_name} 에프씨님! 마스터가 당신의 성공을 진심으로 응원합니다! 정당한 보상을 실천하십시오!"
        st.success(f"### {msg}")
