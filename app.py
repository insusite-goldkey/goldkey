...
# 1. 에러 완전 해결: 모델명을 'gemini-1.5-flash'로 최적화하여 404 에러 종결.
# 2. UI 동선 개편: 분석 결과 및 에러 메시지를 '즉시 분석 버튼' 직하단에 상시 배치.
# 3. 섹션 통합 절대 금지: 1~15번 전 섹션은 물리적 구분선으로 독립성을 수호한다.
# 4. 리스크 관리: 설계사 구상권 보호를 위해 보험업법 벌칙 규정은 답변에서 절대 제외한다.
# 5. 서류 삭제 로직: 2단계 업로드 섹션에 시인성 좋은 빨간색 '전체 삭제' 버튼 배치.
...
2. 법리 결합: 판례 2001다1480 및 상법 근거를 고객의 구체적 사건 내용과 유기적으로 매칭하라.
...
def load_stt_engine():
    ...
    <script>
        window.startRecognition = function() {
            window.speechSynthesis.cancel();
            var msg = new SpeechSynthesisUtterance("마스터가 경청 중입니다.");
            window.speechSynthesis.speak(msg);
...
# ==========================================
# [SECTION 4.5] 마스터 통합 상담 센터 (UI 동선 혁신)
# ==========================================
st.divider()
...
with col_input:
    main_consult_content = st.text_area(
        "📝 마스터 통합 상담창 (내용 구현)", 
        height=320, 
        placeholder="상담 내용을 입력하세요. 입력 후 바로 아래 버튼으로 분석이 가능합니다.",
    ...
    # [🚨 관리자 지시 반영: 에러 및 결과창 전용 컨테이너]
    # 분석 요청 버튼 바로 아래에 결과 혹은 에러가 시인성 좋게 나타납니다.
...
# ==========================================
# [SECTION 8-10] 분석 섹션 분리 (질병/헬스케어/법리)
# ==========================================
st.divider()
    ...
    with container:
        ...
        with st.spinner("마스터가 전 섹션을 통합 분석 중입니다..."):
            try:
                ...
                [판례 적용]: 2001다1480 근거로 보험사 대응 논리 생성. (설계사 구상권 관련 내용은 절대 언급 금지)
                ...
                # [🚨 관리자 요청 반영] 에러 창을 분석 버튼 바로 아래(container)에 시인성 좋게 배치
                st.error(f"🚨 분석 시스템 연결 오류: {e}")
                st.warning("💡 조치 방법: API 키 유효성을 확인하고 잠시 후 다시 시도하십시오. 모델 경로를 표준 명칭으로 강제 고정했습니다.")

# 상단 분석 트리거
if quick_analyze:
    run_master_analysis(main_output_container)
...
if st.button("🏆 관리자 이세윤 & 전문 FC 성공 응원"):
    st.balloons()
    msg = f"{user_name}님! 마스터의 지능으로 최고의 성과를 거두십시오!"
