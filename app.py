...
# 1. 무손실 원칙: 사이드바 혜택, 민원, 연금, 그림 등 누락된 모든 섹션 복구.
# 2. 섹션 독립: 1~15번 전 섹션을 st.divider()로 물리적 완전 분리 배치.
# 3. 마스터 프리패스: admin / gold1234로 즉시 로그인 (회원 DB 오류 방지).
# 4. 기술 해법: Grounding(구글검색) 및 JSON DB 완벽 이식.
...
# --------------------------------------------------------------------------
# [SECTION 3] 사이드바: 마스터 회원 센터 (혜택 및 가이드 복구)
# --------------------------------------------------------------------------
with st.sidebar:
    st.header("🔑 마스터 회원 센터")
...
# --------------------------------------------------------------------------
# [SECTION 4.5] 마스터 통합 상담 센터 (즉시 분석 버튼 복구)
# --------------------------------------------------------------------------
st.divider()
...
home_f = st.number_input("주택자금(만원
