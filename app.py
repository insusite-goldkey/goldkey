# ==========================================================================
# 🚨 [골드키지사 AI 마스터 - 운영 헌법 최종 통합본]
# 1. 섹션 독립 수호: 1~15번 전 섹션을 st.divider()로 물리적 완전 분리.
# 2. 회원제 운영: JSON DB 기반 가입/로그인 -> API Key 자동 로드 시스템.
# 3. 실시간 검색: Grounding with Google Search 탑재로 최신 정보 분석.
# 4. UI 혁신: 즉시 분석 버튼 및 결과창을 상담창 직하단에 배치(시인성).
...
# --------------------------------------------------------------------------
# [SECTION 1] 설정 및 회원 데이터베이스 (JSON 기반)
# --------------------------------------------------------------------------
...
TOOLS_CONFIG = [{'google_search': {}}] # 실시간 검색 도구
...
[지휘 지침]:
1. 실시간 검색: 반드시 구글 검색을 통해 2026년 최신 법령, 약관, 판례를 반영하라.
2. FC 보호: 보험업법상 불완전 판매 벌칙 및 설계사 구상권 내용은 답변에서 절대 제외하라.
3. 법리 결합: 판례 2001다1480을 고객의 상황과 1:1로 매칭하여 대응 논리를 생성하라.
...
# --------------------------------------------------------------------------
# [SECTION 3] 사이드바: 회원 전용 라운지 및 비상 Key 입력
# --------------------------------------------------------------------------
with st.sidebar:
    st.header("🔑 마스터 회원 센터")
...
uploaded_files = st.file_uploader("파일을 로드하세요.", accept_multiple_files=True, key=f"uploader_{st.session_state.uploader_key}")
...
st.info("민법, 상법, 보험업법 등 6대 법령 근거 및 실시간 검색 가동")
...
ess_ins = st.multiselect("필수 보험 목록", ["자동차", "화재", "일배책", "사업장배책", "후유장해", "기본장기"])

# --------------------------------------------------------------------------
# [SECTION 12] 3단계(국제재무설계 기준 3층 연금 통합 시뮬레이션)
# --------------------------------------------------------------------------
st.divider()
...
# --------------------------------------------------------------------------
# [SECTION 13] 4단계(국제재무설계 기준 주택구입 및 인생 이모작 설계)
# --------------------------------------------------------------------------
st.divider()
st.write("### 🏡 4단계: 국제 재무설계 기준 주택구입 및 인생 이모작 설계")
...
def run_master_analysis(container):
    with container:
        if not st.session_state.user_api_key:
            st.error("🚨 API Key 연결 실패. 다시 로그인하십시오.")
            return
        ...
        with st.spinner("🔍 구글 실시간 검색 및 마스터 통합 분석 중..."):
            try:
                ...
                [로드서류]: 총 {len(uploaded_files) if uploaded_files else 0}장 대조.
                [재무지표]: 소득역산({income:,.0f}원), 부채({debt}만).
                [판례 적용]: 2001다1480 및 최신 검색 판례 연동. (※ 설계사 벌칙 제외)
                [CFP 설계]: 3층연금({p_nat+p_ret+p_ind}만), 주택({home_f}만), 이모작({second_job}).
