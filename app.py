# ==========================================================================
# 🚨 [삭제/수정 절대 금지] 골드키지사 AI 마스터 전체 통합 시스템 (섹션 1~10 완본)
# 본 코드는 이세윤 설계사의 전문 로직과 '수예' 음성 엔진이 포함된 마스터본입니다.
# 404 에러 방지 로직과 모바일 최적화, 모든 가이드 튜토리얼이 포함되어 있습니다.
# ==========================================================================
...
with st.expander("💡 [필독] 실전 보상 & 민원/사인거절/횡포 대응 튜토리얼", expanded=False):
    ...
    with t_col1:
        ...
        if st.button("판례 상세 보기"):
            st.write("🔎 **판례 2001다1480:** 합의 당시 예상 못한 중대 후유증은 추가 청구가 가능하다는 대법원 원칙.")
    with t_col2:
...
# ==========================================
# [SECTION 7] 전문가 그룹 통합 분석 (404 완벽 해결)
# ==========================================
st.divider()
...
if st.button("🔍 전문가 그룹 통합 분석 시작 🚀", use_container_width=True, type="primary"):
    if uploaded_files or user_question:
        with st.spinner("수예가 리포트를 작성 중입니다..."):
            try:
                # [404 해결] SDK 기본 안정화 모델 호출
