...
with col_input_area:
    ...
    with col_btn_desc:
        if st.button("🎤 음성 인식 질문 가이드", use_container_width=True):
            st.toast("아래 창에 질문을 입력한 뒤 분석 요청을 눌러주세요.")
            
    user_question = st.text_area("❓ 전문가에게 물어볼 내용을 상세히 적어주세요", height=230, placeholder="예: 이 고객의 보장 공백을 메울 수 있는 전략을 판례 근거로 설명해줘.")

with col_ai_img:
    st.write("") # 상단 여백
    # 깃허브에 올린 ai_expert.png 이미지를 불러옵니다.
    # 주소 형식: https://raw.githubusercontent.com/[사용자아이디]/[저장소이름]/main/ai_expert.png
    # 설계사님의 아이디가 포함된 주소로 자동 추정합니다.
    st.image("https://github.com/seyun-goldkey/goldkey/blob/main/ai_expert.png?raw=true", 
             caption="골드키지사 전담 AI 마스터", use_container_width=True)
...
with col_success_btn:
    if st.button("🚀 FC님 AI와 함께 첨단 보험상담의 주역이 되세요 (성공 업데이트)", use_container_width=True):
        st.balloons()
