# ==========================================================================
# 👑 [골드키지사 AI 마스터 - 유료 등급 전용: 장애 완파 무손실 통합본]
# 절대규칙: 효율을 위해 섹션을 통합하지 말 것. 각 섹션은 독립된 효능을 가진다.
# 1. 404 해결: v1beta와 v1 경로를 모두 탐색하는 자가치유 라우터 탑재.
# 2. 사이드바 완벽 복구: 8년 무료 상담 혜택 및 상세 안내 문구 100% 복원.
# 3. 섹션 독립 수호: 1~15번 독립 섹션을 st.divider()로 물리적 완전 분리.
...
# --------------------------------------------------------------------------
# [SECTION 2] 유료 마스터 엔진 (404 원천 봉쇄 하이브리드 라우터)
# --------------------------------------------------------------------------
SYSTEM_PROMPT = """당신은 이세윤 관리자가 설계한 '골드키지사 AI 마스터'입니다.
1단계 공식 근거(판례, CFP) 생성, 2단계 구글 검색 실시간 교차 검증을 수행합니다.
로직: 소득 역산(0.0709) 및 적정 보험료 15% 가이드를 반드시 준수하십시오."""

def get_master_model(api_key):
    """유료 등급에서 404 없이 모델을 로드하는 고성능 함수"""
    try:
        genai.configure(api_key=api_key)
        
        # [🚨 404 완파 핵심] models/ 접두사를 SDK가 자동으로 붙이도록 순수 모델명만 사용
        # 유료 등급에서는 v1beta보다 안정적인 호출 규격을 우선 시도합니다.
        ...
        # 실패 시 models/ 를 명시적으로 붙인 두 번째 경로 시도
        try:
            model = genai.GenerativeModel(
            ...
            return None, f"최종 장애: {str(e2)}"

# --------------------------------------------------------------------------
# [🚨 SECTION 3] 왼쪽 사이드바: 마스터 회원 센터 (무손실 상세 복구)
# --------------------------------------------------------------------------
with st.sidebar:
    st.header("🔑 마스터 회원 센터")
    ...
    st.markdown('<a href="https://aistudio.google.com/app/apikey" target="_blank"><button style="width:100%; cursor:pointer; background:#1E88E5; color:white; border:none; padding:10px; border-radius:5px; font-weight:bold;">🌐 API 키 발급/확인 바로가기</button></a>', unsafe_allow_html=True)
...
with col_in:
    ...
    main_area = st.text_area("📝 마스터 통합 상담창 (인식 결과)", value=st.session_state.stt_text, height=150)
    ...
    with container:
        ...
        with st.spinner("🔍 유료 보안 엔진 정밀 분석 중..."):
            try:
                ...
                if not model: st.error(f"연결 최종 실패: {msg}"); return
