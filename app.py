...
""".strip()
    ...
    def _validate_response(answer: str, product_key: str, result_key: str = "") -> str:
        """포스트프로세싱: 금지 키워드 감지 + 금소법 문구 치환 + 내부 Verifier 체크"""
        # ── 0. 금소법 단정적 문구 자동 치환 ────────────────────────────────
    ...
    def run_ai_analysis(c_name, query, hi_premium, result_key, extra_prompt="", product_key=""):
        ...
        with st.spinner("골드키AI마스터 분석 중..."):
            try:
                ...
                if _pref_ins and "선택 안 함" not in _pref_ins:
                    _ins_directive = (
                        f"\n\n[선호 보험사 가중치 지시: {_pref_ins} — 금소법 비교안내 의무 이행 모드]"
                        f"\n• 분석 시 {_pref_ins}의 상품을 우선 검토하고, 해당사 상품의 강점(특약·보장범위·인수조건)을 구체적으로 발굴하세요."
                        f"\n• {_pref_ins} 상품이 고객에게 유리한 데이터 근거(보장범위·부지급률·특약구성·인수 유연성)를 명시하세요."
                        f"\n• '현시점 상담 보험사 중 우수한 조건을 보유하고 있습니다' 형식의 법률 권장 문구를 사용하세요."
                        f"\n\n[금소법 제20조 비교안내 의무 — 필수 이행]"
                        f"\n• 답변 하단에 반드시 다음 형식의 [시장 표준 비교표]를 마크다운 표로 생성하세요:"
                        f"\n  | 구분 | {_pref_ins} (선택사) | 시장 표준 | 비고 |"
                        f"\n  |---|---|---|---|"
                        f"\n  | 주요 보장 범위 | (분석 내용) | (시장 평균) | (차별점) |"
                        f"\n  | 인수 조건 | (분석 내용) | (시장 평균) | (차별점) |"
                        f"\n  | 핵심 특약 | (분석 내용) | (시장 평균) | (차별점) |"
                        f"\n• 비교표 아래에 '타사 대비 {_pref_ins}를 선택해야 하는 데이터적 근거'를 1~2줄로 요약하세요."
                        f"\n• 단, 타사 상품의 단점을 과장하거나 허위로 기술하지 마세요. (금소법 제22조 허위·과장 광고 금지)"
                    )
                else:
                    _ins_directive = (
                        "\n\n[선호 보험사: 중립 분석 — 금소법 적합성 원칙 준수]"
                        "\n• 특정 보험사 편향 없이 시장 전체를 기준으로 객관적으로 분석하세요."
