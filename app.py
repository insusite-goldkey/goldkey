...
# 1. 섹션 통합 절대 금지: 모든 섹션(1~15)은 독립 번호와 물리적 영역을 유지한다.
# 2. 브랜딩: "보험 가입 상담 부터 보험금 분쟁 대응까지- 보험 AI전문 마스터 통합 시스템"
# 3. 사이드바 상시 노출: 회원 혜택 및 API 가이드/튜토리얼은 Expander 없이 상시 표현한다.
# 4. 상담창 고도화: 대형 창(height=320) + 직접/음성 2버튼 + 마스터 전문가 이미지.
# 5. 서류 로드 로직: 수량/유형 자동 판별 상태창 및 빨간색 '전체 삭제' 버튼 탑재.
# 6. 명칭 고정: 2단계(국제재무설계 기준...), 3단계(국제 재무설계 기준...).
# 7. 리스크 관리: 설계사 구상권 관련 보험업법 벌칙 규정은 답변에서 절대 제외.
# ==========================================================================
...
[지휘 지침]:
1. 금기 사항: 보험업법상 불완전 판매 벌칙 및 설계사 구상권 관련 내용은 답변에서 절대 제외하라.
2. 보상 대응: 판례 2001다1480 및 상법 제638조의3 근거로 법리 조문과 고객 사건을 결합하여 설명하라.
3. 분석 강령: 건보료 0.0709 소득 역산, 암 담보 합산 금지, CFP® 6단계 프로세스 준수.
"""
...
def load_stt_engine():
    ...
    <script>
        window.startRecognition = function() {
            window.speechSynthesis.cancel();
            var msg = new SpeechSynthesisUtterance("마스터가 경청 중입니다. 말씀하세요.");
            window.speechSynthesis.speak(msg);
...
# ==========================================
# [SECTION 4.5] 마스터 통합 상담 센터 (2모드)
# ==========================================
st.divider()
st.subheader("📡 마스터 통합 상담 센터")
col_input, col_img = st.columns([7, 3])
with col_input:
    main_consult_area = st.text_area("📝 마스터 통합 상담창 (내용 구현)", height=320, placeholder="보험 가입, 민원, 분쟁 대응 등 모든 상담 내용을 입력하세요.")
...
with st.expander("💡 실전 보상 & 민원 대응 튜토리얼 (판례 2001다1480)", expanded=False):
    st.write("보험사의 설명의무 위반 시 법률 조문과 개별 사건을 연관시켜 보상 청구 논리를 생성하는 전문 로직이 탑재되어 있습니다.")
...
# ==========================================
# [SECTION 7] 2단계: 자료 및 증권 업로드 (로드 확인 및 삭제 로직)
# ==========================================
st.divider()
...
if st.button("🔍 전 섹션 마스터 통합 분석 시작 🚀", use_container_width=True, type="primary"):
    ...
    with st.spinner("모든 섹션의 데이터를 유기적으로 분석 중입니다..."):
        try:
            ...
            [보상논리]: 판례 2001다1480 근거 보상 청구 논리 생성. (※ 보험업법 벌칙 제외)
