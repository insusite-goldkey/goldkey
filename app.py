...
- **변경 책임:** 회원정보 변경으로 인해 발생하는 모든 결과(로그인 불가, 데이터 접근 오류 등)에 대한 **책임은 전적으로 해당 회원 본인에게 귀속**됩니다.
- **운영자 면책:** 회원이 직접 변경한 정보의 오류·분실로 인한 서비스 이용 불가에 대해 운영자는 일체의 책임을 지지 않습니다.
...
⚠️ <b>책임 고지</b><br>
이름 변경으로 인해 발생하는 모든 결과(로그인 오류, 데이터 접근 불가 등)에 대한
<b>책임은 전적으로 본인에게 귀속</b>됩니다.<br>
운영자는 회원이 직접 변경한 정보의 오류·분실에 대해 일체의 책임을 지지 않습니다.
</div>""", unsafe_allow_html=True)
