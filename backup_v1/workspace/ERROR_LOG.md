...
### [E008] 로그인 보안 취약 — 전화번호 단독 인증 방치
- **발생일**: 2026-02-28
- **증상**: 이름 + 전화번호만으로 로그인 가능 → 사실상 보안 없음
- **원인**: 초기 구현이 단순 연락처 일치 확인만 수행, 2차 인증 없음
- **수정 (2026-02-28)**: **통합 하이브리드 보안 로그인 시스템** 도입
  - **Phase A** (이름+연락처 확인 → 6자리 OTP 발급)
  - **Phase B** (OTP 인증 → 보안 방식 최초 설정)
  - **Phase SETUP** (생체/패턴/PIN 다중 선택 등록)
  - **Phase C** (이후 실전 로그인 — 생체 우선 + Fallback)
- **보안 방식 3종**:
  | 방식 | 설명 | 색상 |
  |---|---|---|
  | 👆 생체 인식 | 지문/Face ID (Primary 기본값) | 파랑 |
  | ⬛ 패턴 | 3×3 그리드 순서 입력 (최소 4점) | 보라 |
  | 🔢 PIN | 6자리 숫자 패드 | 청록 |
- **Fallback 시스템**: 등록된 방식 중 현재 모드가 아닌 것을 하단 버튼으로 즉시 전환
- **세션 관리**:
  - `_lp`: 현재 단계 (A/B/SETUP/C)
  - `_lp_methods`: 등록된 보안 방식 dict
  - `_sec_methods`: 로그인 완료 후 이력 유지 (다음 접속 시 SETUP 스킵)
- **예방책**:
  - 신규 회원: 첫 로그인 시 반드시 SETUP 단계 통과
  - 기존 방식 등록 이력(`_sec_methods`) 있으면 SETUP 스킵 → Phase C 직행
  - 브루트포스 잠금(`_LoginGuard`)은 Phase A에서 그대로 유지

---

### [I009] 면책 조항 Sticky Footer 추가 — 섹터 1000·3000
- **작업일**: 2026-02-28
- **대상 섹터**:
  - **1000 (home)**: `app.py` line ~10527 — 이미 존재 확인 (추가 불필요)
  - **3000 (cancer)**: `app.py` line ~15321 — `st.stop()` 직전에 신규 삽입
- **문구 (cancer 삽입분)**:
  > 본 앱의 AI 분석 결과는 보조 지표일 뿐 법적 효력이 없습니다. 최종적인 보험 가입·해지·리모델링 결정은 반드시 전문 자격을 갖춘 설계사와 상담하시기 바랍니다. 앱 사용으로 인한 결과의 책임은 사용자 본인에게 있습니다.
- **스타일**: `#1a1a2e` 배경, `0.72rem` 소형 폰트, 앰버 강조 타이틀
- **home 기존 문구**: AI 할루시네이션 면책·공인 설계사 확인 권고 포함 → 동일 취지 이미 충족
