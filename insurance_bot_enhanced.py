...
# -------------------------------------------------------------------------- 
# [SECTION 2] 지능형 온디맨드 RAG 엔진 (마스터 파이프라인 보완)
# -------------------------------------------------------------------------- 
@st.cache_resource
...
class MasterRAGPipeline:
    ...
    def extract_metadata(self, text):
        """[기계적 분석] 텍스트 내에서 회사명과 연도를 자동 추출"""
        # 1. 연도 추출 (정규표현식 활용)
        years = re.findall(r'20\d{2}', text[:1000])  # 문서 상단 1000자 내에서 찾음
        doc_year = years[0] if years else "Unknown"
        
        # 2. 회사명 추출 (핵심 키워드 매칭)
        companies = ["삼성화재", "현대해상", "DB손해보험", "KB손해보험", "메리츠화재", 
                   "삼성생명", "교보생명", "한화생명", "KB생명", "신한생명",
                   "롯데손해보험", "흥국화재", "동부화재", "MG손해보험"]
        doc_company = "General"
        ...
        # 3. 문서 유형 추출
        doc_types = ["약관", "상품설명서", "인수심사기준", "지급기준", "판매공시", "안내서"]
        doc_type = "General"
        for dtype in doc_types:
            if dtype in text[:300]:
                doc_type = dtype
                break
                
        return doc_company, doc_year, doc_type

    def sync_data(self, uploaded_files):
        """관리자 전용: 약관 지식베이스 구축 (메타데이터 포함)"""
        ...
        for file in uploaded_files:
            try:
                ...
                # [자동 분류 공정 추가]
                company, year, doc_type = self.extract_metadata(text)
                ...
                st.info(f"📄 {file.name}: {company} ({year}) - {len(chunks)}개 조각 생성")
    ...
    def retrieve(self, query, k=3):
        ...
        try:
            ...
            return "\n".join(relevant_chunks), relevant_metadatas
        except Exception as e:
    ...
    def get_knowledge_stats(self):
        ...
        try:
            with open(self.json_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                return data.get("total_documents", 0), data.get("total_chunks", 0), data.get("last_updated", "Unknown")
        except:
            return 0, 0, "Unknown"
...
def analyze_with_ai(query, customer_name="고객", rag_context="", rag_metadata=[]):
    try:
        client = get_client()
        ...
        if rag_context:
            ...
            metadata_info = ""
            if rag_metadata:
                sources = set([meta.get('company', 'Unknown') for meta in rag_metadata])
                years = set([meta.get('year', 'Unknown') for meta in rag_metadata])
                metadata_info = f"\n\n[참조 출처: {', '.join(sources)} ({', '.join(years)})]"
            
            master_instruction = f"""당신은 30년 경력의 지능을 가진 '마스터 AI'입니다. 정중한 '하십시오체'를 사용하고 실시간 정보를 기반으로 CFP 수준의 리포트를 작성하세요.
            
            아래 전문 자료를 참고하여 답변하세요:
            {rag_context}{metadata_info}
            """
        else:
    ...
    with st.sidebar:
        ...
        if st.session_state.get('user_name') in ["이세윤", "admin"]: 
            with st.expander("🛠️ RAG 지식베이스 로드 (관리자)", expanded=False):
                # 지식베이스 통계 표시
                total_docs, total_chunks, last_updated = rag_engine.get_knowledge_stats()
                st.info(f"📊 현재 지식베이스: {total_docs}개 문서, {total_chunks}개 조각")
        ...
        if st.button("🚀 마스터 분석 실행", type="primary", use_container_width=True):
            ...
            if judge_needs_rag(query):
                with st.status("🔍 심층 약관 분석 중...", expanded=True) as status:
                    ...
                    context, metadata = rag_engine.retrieve(query)
                    st.write("추출된 약관 근거와 실시간 정보를 대조합니다...")
                    
                    result = analyze_with_ai(query, "고객", context, metadata)
                    if result:
                        ...
                        if context and context != "로드된 약관 지식이 없습니다.":
                            with st.expander("📚 참조된 전문 자료"):
                                ...
                                if metadata:
                                    st.write("**출처 정보:**")
                                    for meta in metadata[:3]:  # 상위 3개만 표시
                                        st.write(f"- 📄 {meta.get('source', 'Unknown')}")
                                        st.write(f"  🏢 {meta.get('company', 'Unknown')} ({meta.get('year', 'Unknown')})")
                                        st.write(f"  📋 {meta.get('type', 'Unknown')}")
