...
## 3. Goldkey AI 태블릿 — 고객 상세 화면 UI 기획 (3-데크 레이아웃)
...
### 3-2. 좌측 사이드바 — [Core] 주라인

> 어떤 탭을 보든 항상 고정 노출. 고객의 핵심 식별 정보.

```
┌──────────────────────┐
│  ★ 이세윤 (45세/남)   │  ← is_favorite 별표
│  VIP · 동창회         │  ← management_tier + community_tags
├──────────────────────┤
│  📞 010-xxxx-xxxx    │  ← 터치 시 전화 연결
│  🏠 서울 강남구       │  ← address
│  💼 IT 전문가         │  ← job
├──────────────────────┤
│  상태: [진행중 ▼]     │  ← status 드롭다운
│  최근상담: 2026-03-10 │  ← last_consulted_at
├──────────────────────┤
│  [상담 시작]          │
│  [증권 스캔]          │
└──────────────────────┘
...
**컴포넌트명**: `CustomerCoreSidebar`
**데이터 소스**: `gk_people` Core 컬럼 전체

---

### 3-3. 중앙 메인 영역 — 탭 시스템

#### 탭 A — [1차 라인] 연간 계약 보드 `AnnualCycleTab`
...
│  현재: 삼성화재   │  [등록하기]                  │
├──────────────────┴──────────────────────────────┤
│  ☑ 갱신 안내 문자 발송   □ 견적 비교 완료         │
│  갱신 메모: renewal_memo 표시                    │
└─────────────────────────────────────────────────┘
```

**데이터**: `auto_renewal_month`, `fire_renewal_month`, `renewal_memo`

---

#### 탭 B — [2차 라인] 전략 케어 & AI 리포트 `StrategicCareTab`
...
├─────────────────────────────────────────────────┤
│  라이프사이클 타임라인                           │
│  ● 결혼기념일 D-45   ● 장녀 취업 예정 6월       │
├─────────────────────────────────────────────────┤
│  직업 위험도: 중위험 (IT 전문가)                 │
│  핵심 관리 포인트: key_customer_note             │
├─────────────────────────────────────────────────┤
│  AI 보장 분석 리포트 (최신)          [PDF 보기]  │
│  gk_scan_results 연동 요약본                    │
└─────────────────────────────────────────────────┘
```

**데이터**: `management_tier`, `key_customer_note`, `lifecycle_events`, `gk_scan_results`

---

#### 탭 C — [3차 라인] 생활 활동 & 인맥 지도 `SocialNetworkTab`

```
┌─────────────────────────────────────────────────┐
│  🏟️ 3차 라인 — 생활 활동 & 개척                 │
├─────────────────────────────────────────────────┤
│  소속 모임: [조기축구회 5명▶] [강남동창회 12명▶] │
├─────────────────────────────────────────────────┤
│  소개 계보                                       │
│  [김철수] ─소개─▶ [이세윤] ─소개─▶ [박영희]     │
├─────────────────────────────────────────────────┤
│  개척 단계: ●●●○ 자료전달 완료 → 상담 예약 중   │
│  prospecting_stage: follow-up                   │
└─────────────────────────────────────────────────┘
```

**데이터**: `community_tags`, `referral_from`, `prospecting_stage`, `activity_group`

---

### 3-4. 우측 보조 영역 — 상담 히스토리 `ConsultHistoryPanel`
...
**데이터**: `gk_schedules`, `memo`

---

### 3-5. 그림자 이식 규칙 (CRM 핸드폰 배치 기준)

> **Phase 2 이식 시 이 규칙을 따른다.**

```
모 앱 (태블릿 3-데크)          →   자 앱 (핸드폰 세로 1컬럼)
─────────────────────────────────────────────────────────────
CustomerCoreSidebar (좌)       →   상단 고정 카드 (축약형)
AnnualCycleTab (탭A, 1차)      →   섹션 1: 연간 갱신 카드
StrategicCareTab (탭B, 2차)    →   ⛔ Phase 2 제외 (AI 앱 전용)
SocialNetworkTab (탭C, 3차)    →   섹션 2: 모임/개척 카드
ConsultHistoryPanel (우)       →   섹션 3: 상담 메모 카드
```

- **탭B(2차)**는 AI 연산이 필요하므로 CRM에서는 **읽기 전용 요약** 표시
- 컴포넌트 파일은 동일, `isTablet` prop으로 레이아웃만 분기:
  ```javascript
  // 모 앱: isTablet=true  → 3-데크 가로 배치
  // 자 앱: isTablet=false → 세로 스크롤 카드 배치
  <CustomerDetailScreen isTablet={isTablet} />
  ```
