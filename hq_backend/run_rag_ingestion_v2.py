"""
[Phase 5 고도화] 지능형 RAG 임베딩 파이프라인 실행 스크립트
source_docs 폴더의 모든 PDF를 자동으로 처리하고 상세 보고서 생성

실행 방법:
    python hq_backend/run_rag_ingestion_v2.py
...
sys.path.insert(0, str(project_root / "hq_backend"))
...
# 파이프라인 import
try:
    from core.rag_ingestion_v2 import IntelligentRAGPipeline
except ImportError as e:
    print(f"\n❌ 모듈 import 실패: {e}")
    print("다음 명령으로 필수 패키지를 설치하세요:")
    print("pip install langchain-text-splitters langchain-community pypdf openai supabase")
    sys.exit(1)

# 소스 디렉토리 확인
source_dir = project_root / "hq_backend" / "knowledge_base" / "source_docs"

if not source_dir.exists():
    print(f"\n❌ 소스 디렉토리 없음: {source_dir}")
    sys.exit(1)
