...
## 2.0 중앙 컨트롤센터 (GCS 사령부) — 머리 1 · 몸통 2

**기본틀:** **머리 1개**(중앙 두뇌) + **몸통 2개**(실행 앱) — **몸통 = HQ + CRM**.

| 구분 | 역할 |
|------|------|
| **머리 — GCS 중앙 컨트롤센터(사령부)** | 앱이 알아야 할 **모든 지식·운영·보고·분석 원천**을 통제: 구성·설치·운영·보고서·카카오 보고서 작성·전송, 법률·손해사정·보험금·라이프사이클·노후·세금·화재·자동차·신규가입 상담, 컨설팅용 설계사 대화법 등. **여기서 수정하면** 연계 모듈이 같은 기준을 쓰도록 한다. |
| **몸통 — HQ (`app.py`)** | 정밀 분석·탭·사령부 자료 **연계·실행** |
| **몸통 — CRM (`crm_app.py`)** | 현장·일정·딥링크·경량 UI로 **연계·실행** |

**설계·개발 절차:** 설계자가 「어떤 기능을 업데이트」라고 하면, (1) 그 기능과 연결된 **컨트롤 타워(사령부) 쪽을 먼저** 수정하고, (2) 그 변경을 **지정 모듈(HQ/CRM 등)과 예열·연결**해 앱 흐름을 맞춘다. HQ/CRM에만 지식을 이중으로 넣어 **사령부와 어긋나게 하지 않는다.**
