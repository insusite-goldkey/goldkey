...
_TERMS_TEXT = """■ Goldkey AI Masters 2026 이용약관

[제1조] 목적
본 약관은 Goldkey_AI_Masters2026(이하 '서비스')의 이용 조건 및 절차, 운영자와 회원의 권리·의무 및 책임 사항을 규정함을 목적으로 합니다.

[제2조] 서비스 이용 조건
• 현재 전체 무료 베타 서비스 운영 중 / 회원 1인당 1일 10회 AI 상담 이용 제한
• 사용 대상: 19세 이상 보험 영업 관련 종사자 및 설계사

[제3조] 서비스 기능 안내
AI 분석 보조 도구 / 세무·법무·상속·증여 관련 참고 정보 제공 / 담당 보험사 및 청구 안내 절차 지원

[제4조] 제한 사용 범위
계정 도용 및 허위 정보 입력 방지 / 서비스를 방해하는 비정상적 접근 및 부당 권한 획득 방지 / 시스템 크롤링 및 자동화 프로그램 사용 금지

[제5조] 개인정보 수집 및 이용
• 수집 항목: 이름, 비밀번호(암호화 저장), 이메일
• 이용 목적: 회원 인증, 이용 한도 관리, 서비스 품질 개선
• 보유 기간: 회원 탈퇴 시 즉시 파기 (단, 관련 법령에 의한 보존 의무가 있는 경우 예외) / 제3자 무단 제공 금지

[제5조의2] 회원 정보 보안 보호
• 비밀번호: SHA-256 단방향 해시(One-Way Hash)로 저장 (관리자 포함 누구도 원문 열람 및 복원 불가)
• 데이터: AES 기반 Fernet 암호화 적용, 세션 종료 시 자동 파기 / 네트워크 전송: TLS(HTTPS) 보안 통신 적용

[제6조] 고객 정보 보안 기준
SHA-256 단방향 암호화 / AES-128 Fernet 암호화 / TLS 전송 보안 적용 / ISO/IEC 27001 정보보안 인증 기준 준수 / GDPR 및 국내 개인정보 보호법 준수

[제6조의2] 마이크 접근 권한
• 음성 입력(STT) 기능 사용 시 마이크 접근 권한 요청 (음성 데이터 서버 미저장)
• 권한 거부 시에도 음성 입력을 제외한 모든 기능 정상 사용 가능
• Web Speech API(Google 제공)를 통한 음성→텍스트 변환 (Google 서버 경유 변환 처리)

[제7조] 고객 정보 처리 기준
• 파기: 탈퇴 요청 시 DB 즉시 파기 / 자동 파기: 세션 종료 시 질문 및 상담 내용 즉시 파기
• 정기 파기: 90일 이상 미접속 시 자동 삭제 / 복구 불가능한 영구 삭제 방식 적용

[제8조] 책임 한계 및 안내
AI 기술을 활용한 보조 분석 도구로서, 모든 분석 결과 및 임시 산출물은 사용자(설계사/상담원)에게 참고용으로 제공됩니다. 보험금 지급 여부를 결정하는 최종 판단은 해당 보험사 담당자의 심사에 따르며, 구체적인 세무·의료·법률 문제는 반드시 해당 전문가와 교차 검증하십시오. 본 서비스는 보험 모집·중개·알선 행위와 독립적인 순수 AI 분석 보조 도구이며, 최종 상담 결과에 대한 책임은 앱 운영자가 아닌 이용자 본인에게 귀속됩니다.

[제8조의2] 회원 정보 변경 및 책임
회원은 이름, 연락처, 비밀번호를 앱 내 기능을 통해 직접 변경할 수 있습니다. 회원이 직접 입력 및 변경한 정보의 오류로 인해 발생하는 불이익은 회원 본인에게 귀속됩니다. 단, 시스템 오류 및 서버 장애로 인한 손해는 운영자가 책임집니다. (장애 문의: insusite@gmail.com / 010-3074-2616)

[제9조] 금융소비자보호법(금소법) 준수 원칙
① 적합성 원칙(제17조): 고객의 연령·소득·위험성향 기반 맞춤 분석 보조
② 특정 보험사 제휴 및 상품 판매 수수료 없음
③ 부당권유행위 금지(제21조): 단정적 표현 자동 감지 및 순화 처리
④ 허위·과장광고 금지(제22조): 약관 및 판례 기반의 객관적 분석 제공
본 서비스는 보험 모집·중개·알선 행위와 무관한 독립적인 AI 분석 보조 도구입니다.

[제10조] 데이터 분리 저장 및 개인정보 주권 보호(하이브리드 아키텍처)
• Public Zone: 공용 보험 약관·의학 정보·법령 데이터 (중앙 공용 서버 보관)
• Private Zone: 회원이 업로드한 고객 상담 기록·증권 등 민감 정보 → 회원 UID에 종속되어 암호화 저장
• 기술적 접근 통제: 운영자 및 관리자도 Private Zone 접근 절대 불가 (IAM 403 차단 적용) / AES-256-GCM 암호화 저장
• 탈퇴 시 해당 회원의 모든 파일, 메타데이터, 계정 정보는 완전 파기(복구 불가) 처리됨

[제11조] 준거법 및 관할
본 약관은 대한민국 법률에 따라 규율되며, 본 서비스와 관련된 분쟁의 관할 법원은 민사소송법상의 관할 법원으로 합니다.

[제12조] 내보험다보여 서비스 연계 지원
본 서비스는 신용정보원 '내보험다보여(www.credit4u.or.kr)' 조회 결과를 상담 보조 자료로 활용할 수 있도록 지원합니다. 본 앱은 한국신용정보원 및 금융감독원과 직접적인 제휴·위탁 관계가 없는 독립적인 보조 도구입니다.

[제13조] 카카오톡 메시지 서비스 보안 및 권한 안내
• 서비스명: 골드키 마스터 AI 보고서 전송 시스템
• ① 보안 정책: 본 시스템은 사용자의 대화 내용을 열람하거나 친구 목록을 수집하지 않습니다. 요청 권한은 talk_message(메시지 전송) 단 1개입니다.
• ② 데이터 처리: 송신 데이터는 TLS 보안 처리되며, 전송 즉시 파기됩니다. 서버에 메시지 내용이 저장되지 않습니다.
• ③ 권한 거부: 카카오톡 앱 → 설정 → 카카오계정 → 연결된 서비스 관리에서 언제든 권한을 철회할 수 있습니다.

[제14조] 🔒 보안 기준 준수 (Security Standards)
• ISO/IEC 27001 정보보안 관리 인증 기준 준용
• GDPR 및 국내 개인정보보호법 가이드라인 준수
• TLS 1.3 전송 구간 암호화 적용 (서버-클라이언트 통신 보호)
• AES-256 Fernet 기반의 서버 데이터베이스 암호화
• SHA-256 단방향 해시 알고리즘 적용 (비밀번호 저장)
• 세션 종료 시 민감 정보 메모리 즉시 파기 (임시 데이터 저장 원천 차단)
• 외부 API 전송 시 개인정보 비식별화(마스킹) 필수 적용 (이름, 휴대전화, 주민등록번호 등)

[제15조] 외부 캘린더 연동 및 정보 활용 범위
본 서비스는 회원의 편의를 위해 회원의 명시적 동의 하에 외부 캘린더(Google Calendar, Apple Calendar 등)의 일정 정보를 API(OAuth 2.0)를 통해 연동할 수 있습니다. 연동된 외부 일정 정보는 앱 내 일정 관리 및 알림 목적으로만 활용되며, 제3자 제공, 마케팅 활용, AI 학습 데이터 사용은 일절 금지됩니다.
• 연동 방식: OAuth 2.0 표준 인증 (회원 인증 정보 서버 미저장)
• 데이터 수집: 회원의 명시적 클릭 시에만 동기화 실행 (자동 백그라운드 스크래핑 금지)
• 권한 철회: 회원은 설정 메뉴에서 언제든 연동을 해제하고 관련 데이터를 파기할 수 있으며, 연동 해제 시 서버에 임시 저장된 캘린더 데이터는 영구 삭제됩니다.

[제16조] 데이터 종속성 및 관리자 접근 통제 원칙 (Admin-Blind Zero-Knowledge)
본 서비스는 회원이 입력한 고객 DB 및 모든 상담 정보에 대해 'Zero-Knowledge(영지식)'에 준하는 보안 정책 및 RLS(Row Level Security)를 적용합니다. 회원의 모든 데이터는 해당 계정(회원 본인)에만 종속되며, 당사의 시스템 관리자, 개발자, 운영자를 포함한 그 어떠한 제3자도 회원의 고객 정보 및 일정 데이터를 임의로 열람, 추출, 처리, 공유할 수 없습니다.
• 기술 구현: Supabase RLS — auth.uid()::text = Agent_id 강제 적용
• 보안 계층: AES-256-GCM (저장 시) / TLS 1.3 (전송 시) / SHA-256 (인증 시)
• 관리자 통제: IAM 접근 제어 (위반 접근 시도 시 감사 로그 기록)
• 법적 근거: 개인정보보호법 제29조(안전조치의무) 및 GDPR Article 25(Privacy by Design) 준수

[제17조] 외부 내보내기 및 권한 연동에 따른 책임
회원이 본 앱의 일정이나 데이터를 외부 플랫폼(Google, Apple 등)으로 내보내기 하거나 연동할 경우, 해당 플랫폼으로 전송된 데이터의 보안 및 관리는 해당 플랫폼의 이용약관 및 개인정보처리방침을 따릅니다. 당사는 다음의 경우에 대해 책임을 지지 않습니다:
① 회원의 계정 관리 소홀(비밀번호 유출 등)로 인한 정보 유출
② 외부 플랫폼 자체의 귀책사유로 인한 서비스 중단 및 데이터 손실
③ 회원이 외부 플랫폼에 임의로 허용한 제3자 앱 권한으로 인한 정보 유출

[제18조] 카카오톡 알림톡 전송 및 제3자 제공 동의
본 서비스는 고객에게 AI 분석 보고서 및 상담 결과를 안내할 목적으로 고객의 휴대전화 번호를 카카오톡 알림톡 API 서버로 전송할 수 있습니다.
① 전송 목적: AI 분석 보고서, 상담 일정 안내, 계약 관련 정보 발송
② 전송 대상: 설계사가 명시적으로 전송을 요청한 고객 (사전 수신 동의 여부 확인 필수)
③ 제3자 제공받는 자: 주식회사 카카오 (카카오 알림톡 서비스 연동)
④ 데이터 보호 조치: 카카오톡 API로 전송되는 메시지 내의 민감한 개인정보(이름 일부, 휴대전화 번호 일부, 주민등록번호 뒷자리 등)는 개인정보보호법 안전조치 의무에 따라 별표(*)로 비식별화(마스킹) 처리되어 발송됩니다.
⑤ 보유 및 파기: 전송 목적 달성 즉시 파기 (단, 관련 법령에 의한 로그 기록은 최대 3년 보관)
⑥ 본 조항은 개인정보보호법 제17조(개인정보의 제공) 및 정보통신망법 제50조(영리목적의 광고성 정보 전송 제한)를 준수합니다.

⚠️ 면책 및 서비스 이용안내 (면책조항)
① 본 앱(Goldkey_AI_Masters2026)은 설계사의 고객 상담 업무를 돕는 보조 도구입니다. 모든 AI 분석 결과는 참고용 지표이며, 확정적인 보장 분석이나 절대적인 청구 기준이 될 수 없습니다.
② 구체적인 보장 내용, 약관 해석, 보험금 청구 가부는 반드시 해당 보험사의 보상 담당자 또는 손해사정사에게 최종 확인하십시오.
③ AI 분석 결과는 기술적 한계로 인한 오류(AI 할루시네이션)가 포함될 수 있으며, 이로 인해 발생하는 어떠한 손해에 대해서도 당사는 법적 책임을 지지 않습니다.
④ 본 앱은 의료, 법률, 세무, 회계, 부동산 등 전문적 진단이나 유권해석을 대체할 수 없으며, 이를 바탕으로 한 최종 판단과 책임은 전적으로 이용자(설계사) 본인에게 있습니다.

• 최종 갱신일: 2026년 3월 31일
■ 서비스명: Goldkey_AI_Masters2026 | 운영자: 이세윤 | 문의: 010-3074-2616 / insusite@gmail.com
⚠️ 위 필수 약관 및 동의 사항 3가지에 모두 체크하셔야 로그인 버튼이 활성화됩니다.
"""
...
def render_auth_screen(
    ...
    _terms_md(
        ...
        "본 서비스는 회원의 편의를 위해 회원의 <b>명시적 동의</b>하에 외부 캘린더(Google Calendar, Apple Calendar 등)의 "
        "일정 정보를 <b>API(OAuth 2.0)</b> 방식을 통해 연동합니다. "
        "수집된 외부 일정 정보는 <b>오직 본 앱 내에서의 스케줄 관리 및 알림 목적으로만 렌더링</b>되며, "
        "제3자 제공·마케팅 활용·학습 데이터 사용은 일체 금지됩니다.<br>"
        "• <b>연동 방식:</b> OAuth 2.0 표준 인증 (회원 자격증명 당사 서버 미저장)<br>"
        "• <b>동기화 트리거:</b> 회원의 명시적 버튼 클릭 시에만 실행 (자동 수집·스크래핑 절대 금지)<br>"
        "• <b>연동 해제(Revoke):</b> 회원은 언제든지 환경설정에서 연동을 해제하고 관련 데이터를 즉시 파기 가능 — "
        "해제 즉시 당사 서버의 해당 연동 데이터는 복구 불가능한 방법으로 완전 삭제됩니다.<br><br>"

        "<b style='color:#0a1628;'>[제16조] 절대적 데이터 폐쇄성 및 관리자 접근 제한 원칙 (Admin-Blind Zero-Knowledge)</b><br>"
        "본 서비스는 회원이 입력한 <b>고객 DB 및 모든 스케줄 정보</b>에 대해 "
        "<b>'Zero-Knowledge(영지식)'에 준하는 절대 폐쇄형 암호화 및 RLS(Row Level Security) 아키텍처</b>를 적용합니다. "
        "회원의 모든 데이터는 <b>해당 계정(회원 본인)에만 독점적으로 종속</b>되며, "
        "당사의 시스템 관리자, 개발자, 운영자를 포함한 <b>그 어떠한 제3자도</b> "
        "회원의 고객 정보 및 일정 데이터를 <b>열람, 추출, 가공, 공유할 수 없습니다.</b><br>"
        "• <b>기술 구현:</b> Supabase RLS — auth.uid()::text = agent_id 정책 강제 적용<br>"
        "• <b>암호화:</b> AES-256-GCM(저장) / TLS 1.3(전송) / SHA-256(인증 해시)<br>"
        "• <b>관리자 접근:</b> IAM 정책으로 기술적 차단 (위반 시 감사 로그 자동 기록)<br>"
        "• <b>법적 근거:</b> 개인정보보호법 제29조(안전조치의무) · GDPR Article 25(Privacy by Design)<br><br>"

        "<b style='color:#0a1628;'>[제17조] 외부 캘린더 내보내기(Export) 및 권한의 위임·책임의 한계</b><br>"
        "회원이 본 앱의 일정을 외부 캘린더로 <b>내보내기(Export)</b> 하거나 동기화할 경우, "
        "외부 플랫폼(Google, Apple 등)으로 전송된 데이터의 보안 및 관리는 "
        "<b>해당 플랫폼의 개인정보 처리방침</b>을 따릅니다. "
        "당사는 다음 경우에 대해 <b>법적 책임을 부담하지 않습니다:</b><br>"
        "① 회원의 <b>계정 관리 소홀</b>(비밀번호 유출 등)로 인한 정보 유출<br>"
        "② 외부 플랫폼 측의 귀책으로 발생한 서비스 중단·데이터 손실<br>"
        "③ 회원이 외부 플랫폼에 부여한 제3자 앱 접근 권한으로 인한 정보 침해<br>"
        "동기화 실행 전 해당 외부 플랫폼의 이용약관 및 개인정보 처리방침을 반드시 확인하십시오.<br><br>"

        "<b style='color:#0a1628;'>[제18조] 카카오톡 메시지 발송 및 개인정보 제3자 제공</b><br>"
        "본 서비스는 보험 리포트·상담 결과 안내 목적으로 고객의 휴대전화번호를 카카오톡 알림톡 API에 전달할 수 있습니다.<br>"
        "① <b>전송 목적:</b> AI 분석 리포트·상담 일정·계약 안내 메시지 발송<br>"
        "② <b>수신자:</b> 설계사가 등록한 고객 (사전 구두 동의 확인 책임은 설계사에게 있음)<br>"
        "③ <b>제3자 제공:</b> 카카오 주식회사(카카오톡 알림톡 플랫폼) — 전송 목적 한정<br>"
        "④ <b>데이터 보호 조치:</b> 카카오톡 API로 전송되는 메시지 내의 민감한 개인정보(이름 일부, 휴대전화 번호 일부, 주민등록번호 뒷자리 등)는 "
        "개인정보보호법 안전조치 의무에 따라 별표(*)로 <b>비식별화(마스킹) 처리되어 발송</b>됩니다.<br>"
        "⑤ <b>보유·파기:</b> 발송 로그는 상담이력 테이블에 암호화 보관, 계약 종료 후 3년 경과 시 자동 파기<br>"
        "⑥ <b>동의 거부 권리:</b> 동의 거부 시 카카오톡 리포트 발송 기능 비활성화 (나머지 기능 정상 이용)<br>"
        "⑦ <b>법적 근거:</b> 개인정보보호법 제17조(제3자 제공) · 정보통신망법 제50조(광고성 정보 전송) 적용<br><br>"
...
div[data-testid="stVerticalBlock"] > div:has(> div[data-testid="stCheckbox"]) {
    ...
    _c8 = st.checkbox(
        "💬 **[카카오톡 선택]** 카카오톡 알림톡 발송 및 개인정보 제3자 제공(카카오)에 동의합니다 (제18조·개인정보보호법 제17조)",
