...
_TERMS_TEXT = """■ Goldkey_Ai_masters2026 이용약관

[제1조 목적]
본 약관은 Goldkey_AI_Master2026 서비스(이하 '서비스')의 이용 조건 및 절차,
...
[제15조 외부 캘린더 정보의 연동 및 활용]
본 서비스는 회원의 편의를 위해 회원의 명시적 동의하에 외부 캘린더(Google Calendar,
Apple Calendar 등)의 일정 정보를 API(OAuth 2.0) 방식을 통해 연동합니다.
수집된 외부 일정 정보는 오직 본 앱 내에서의 스케줄 관리 및 알림 목적으로만 렌더링되며,
제3자 제공·마케팅 활용·학습 데이터 사용은 일체 금지됩니다.
회원은 언제든지 앱 내 환경설정에서 연동을 해제(Revoke)하고 관련 데이터를 즉시 파기할 수 있으며,
해제 즉시 당사 서버에 저장된 해당 연동 데이터는 복구 불가능한 방법으로 완전 삭제됩니다.
- 연동 방식: OAuth 2.0 표준 인증 (회원 계정 정보를 당사 서버에 저장하지 않음)
- 동기화 트리거: 회원의 명시적 버튼 클릭 시에만 실행 (자동 수집·스크래핑 절대 금지)
- 수집 범위: 일정 제목·날짜·시간 (개인 의료·금융정보 등 민감 정보 자동 필터링)

[제16조 절대적 데이터 폐쇄성 및 관리자 접근 제한 원칙 (Admin-Blind Zero-Knowledge)]
본 서비스는 회원이 입력한 고객 DB 및 모든 스케줄 정보에 대해 'Zero-Knowledge(영지식)'에
준하는 절대 폐쇄형 암호화 및 RLS(Row Level Security) 아키텍처를 적용합니다.
회원의 모든 데이터는 해당 계정(회원 본인)에만 독점적으로 종속되며,
당사의 시스템 관리자, 개발자, 운영자를 포함한 그 어떠한 제3자도
회원의 고객 정보 및 일정 데이터를 열람, 추출, 가공, 공유할 수 없습니다.
- 기술 구현: Supabase RLS (auth.uid()::text = agent_id 정책 강제 적용)
- 암호화: AES-256-GCM (저장), TLS 1.3 (전송), SHA-256 (인증 해시)
- 관리자 접근: IAM 정책으로 기술적 차단 (정책 위반 시 감사 로그 자동 기록)
- 법적 근거: 개인정보보호법 제29조(안전조치의무), GDPR Article 25(Privacy by Design)

[제17조 외부 캘린더 내보내기(Export) 및 권한의 위임·책임의 한계]
회원이 본 앱의 일정을 외부 캘린더로 내보내기(Export) 하거나 동기화할 경우,
외부 플랫폼(Google, Apple 등)으로 전송된 데이터의 보안 및 관리는
해당 플랫폼의 개인정보 처리방침 및 보안 정책을 따릅니다.
당사는 다음 경우에 대해 법적 책임을 부담하지 않습니다:
① 회원의 계정 관리 소홀(비밀번호 유출 등)로 인한 정보 유출
② 외부 플랫폼 측의 귀책으로 발생한 서비스 중단·데이터 손실
③ 회원이 외부 플랫폼에 부여한 제3자 앱 접근 권한으로 인한 정보 침해
동기화 실행 전 해당 외부 플랫폼의 이용약관 및 개인정보 처리방침을 반드시 확인하십시오.

[제18조 카카오톡 메시지 발송 및 개인정보 제3자 제공]
본 서비스는 보험 리포트·상담 결과 안내 목적으로 고객의 휴대전화번호를 카카오톡 알림톡 API에 전달할 수 있습니다.
① 전송 목적: AI 분석 리포트·상담 일정·계약 안내 메시지 발송
② 수신자: 설계사가 등록한 고객(사전 구두 동의 확인 책임은 설계사에게 있음)
③ 제3자 제공: 카카오 주식회사(카카오톡 알림톡 플랫폼) — 전송 목적 한정
④ 보유·파기: 발송 로그는 상담이력 테이블에 암호화 보관, 계약 종료 후 3년 경과 시 자동 파기
⑤ 동의 거부 권리: 동의 거부 시 카카오톡 리포트 발송 기능 비활성화 (나머지 기능 정상 이용)
⑥ 개인정보보호법 제17조(제3자 제공), 정보통신망법 제50조(광고성 정보 전송) 적용
"""
